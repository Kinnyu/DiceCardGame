...
export async function handleRoomApi({ method, path = [], body = {}, store, random = Math.random }) {
  ...
  if (!["", "join", "leave", "start", "arrange", "roll", "turn"].includes(route.action)) {
    return response(404, { error: errors.apiNotFound });
  }
  ...
  if (method === "GET" && route.action === "") {
    return response(200, store.decorate({ room: publicRoom(room, { playerId: body.playerId }) }));
  }
...
async function arrangeCards({ room, body, store }) {
  ...
  const result = arrangePlayerCards(room.game, playerId, body.cardIds);
  if (result.error) {
