...
export function selectDraftCard(game, playerId, cardInstanceId) {
  ...
  if (player.selectedDraftCards.length === HAND_SIZE) {
    ...
    const selectedInstanceIds = new Set(player.selectedDraftCards.map((selectedCard) => selectedCard.instanceId));
    player.drawDeck = draftCards
      .filter((candidate) => !selectedInstanceIds.has(candidate?.instanceId))
      .map((candidate) => prepareHandCard(candidate));
  }
...
export function passArrangedCardsRight(game) {
  ...
  const passedDrawDecks = activePlayers.map((player) =>
    (Array.isArray(player.drawDeck) ? player.drawDeck : []).map((card) => ({ ...card }))
  );
  ...
  for (let index = 0; index < activePlayers.length; index += 1) {
    ...
    activePlayers[receiverIndex].drawDeck = passedDrawDecks[index];
