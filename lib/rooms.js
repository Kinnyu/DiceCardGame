...
export const errors = {
  ...
  apiNotFound: "找不到房間 API。",
  gameAlreadyStarted: "遊戲已經開始，暫時不能加入。",
  hostOnly: "只有房主可以開始遊戲。",
  notEnoughPlayers: "至少需要兩位玩家才能開始。",
  unsupportedAction: "不支援這個房間操作。",
  storageNotConfigured:
    "後端房間儲存尚未設定。正式 Vercel 部署需要 `UPSTASH_REDIS_REST_URL` 和 `UPSTASH_REDIS_REST_TOKEN`。"
};
...
export function publicRoom(room) {
  ...
  if (room.game) {
    view.game = publicGameState(room.game);
  }
